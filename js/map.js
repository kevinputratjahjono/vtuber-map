...
//  Handles province highlighting, tooltips, click events
// ============================================================
...
const MapModule = (() => {
  ...
  /* ---- Find province data by GeoJSON feature name ---- */
  // GeoJSON kita pakai properti "PROVINSI"
  ...
  function onEachFeature(feature, layer) {
    ...
    // Tooltip
    layer.bindTooltip(`
      <div style="font-family:'Orbitron',monospace;font-size:10px;color:#00f5ff;margin-bottom:4px;letter-spacing:1px;">
        ${geoName}
      </div>
      <div style="font-family:'Rajdhani',sans-serif;font-size:12px;color:#e8eaf6;">
        ${count} penonton
        ${pd ? `<span style="color:#8892b0;font-size:10px;"> · ${pd.male}♂ / ${pd.female}♀</span>` : ''}
      </div>
    `, {
      sticky: true,
      opacity: 0.95,
    ...
    layer.on({
      mouseover(e) {
        if (hoveredLayer && hoveredLayer !== layer) {
          ...
          hoveredLayer.setStyle({ weight: isS ? 2.5 : 1, color: isS ? '#ff2d9f' : stroke, fillOpacity: 1 });
        }
        hoveredLayer = layer;
        layer.setStyle({
          weight: 2,
      ...
      click() {
        ...
        if (onClickCb && pd) onClickCb(pd, geoName);
      }
    });
  }
  ...
  function init(containerId, data, maxC, breaksArr, clickCallback) {
    ...
    fetch('data/indonesia.geojson')
      ...
      .then(geojson => {
        ...
        console.info('GeoJSON loaded:', geojson.features.length, 'fitur provinsi');
      })
      .catch(err => {
        console.warn('GeoJSON gagal dimuat, pakai marker fallback:', err.message);
        drawFallbackMarkers();
      });
  }

  /* ---- Fallback markers jika GeoJSON tidak tersedia ---- */
  ...
  function drawFallbackMarkers() {
    Object.entries(provData).forEach(([provName, pd]) => {
      ...
      circle.bindTooltip(`<b style="color:#00f5ff">${provName}</b><br>${pd.total} penonton`, { sticky: true });
      circle.on('click', () => {
        selectedProv = provName;
        if (onClickCb) onClickCb(pd, provName);
      });
    });
  }

  /* ---- Highlight a province programmatically ---- */
  function selectProvince(provName) {
    ...
    if (geoLayer) {
      geoLayer.eachLayer(l => {
        ...
        l.setStyle({ fillColor: fill, fillOpacity: 1, color: isSel ? '#ff2d9f' : stroke, weight: isSel ? 2.5 : 1 });
        if (isSel) { l.bringToFront(); map.fitBounds(l.getBounds(), { padding: [40, 40] }); }
      });
    }
  }
